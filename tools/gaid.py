...
# --- какой это исходник ------------------------------------------------------
# Три вёрстки, три профиля. Первые два (гайды про Logic и UAD) опознаются
# по обложке <section class='cover'>. Третий — «справочник»: у него своя
# боковая панель с поиском, разделы набраны h2, подразделы h3, а название
# стоит в единственном h1. Ветки не смешиваем: правка в одной не должна
# ронять другую.
SPRAVOCHNIK = "<section class='cover'>" not in s

# Что уже лежит в мете. Поля, которых нет в исходнике (подводка, область,
# текст плашки «Как читать»), проставляются руками — и должны пережить
# следующий прогон скрипта, иначе правка гайда молча сотрёт их.
PUT_META = os.path.join(KOREN, f'src/data/gaidy/{SLUG}.json')
...
if not SPRAVOCHNIK:
    ...
    # ==========================================================================
    # Профиль «справочник»: h1 — название, h2 — разделы, h3 — подразделы.
    # Обложки нет, зато есть своя боковая панель с поиском и переключателем
    # темы — всё это снимаем, оснастку даёт сайт.
    # ==========================================================================
    kartinki = []

    telo = re.search(r'<article class="article"[^>]*>(.*?)</article>', s, re.S).group(1)

    # якорь «#» перед каждым заголовком — часть чужой оснастки
    telo = re.sub(r'<a class="heading-anchor".*?</a>', '', telo, flags=re.S)

    # название: единственный h1. На сайте его рисует каркас, из тела убираем
    h1 = re.search(r'<h1[^>]*>(.*?)</h1>', telo, re.S)
    nazvanie = tekst(h1.group(1))
    telo = telo[h1.end():]

    # шапка исходника (версии панели и ядра, дата проверки) — во врезку.
    # Для справочника это не украшение: без версий половина полей теряет смысл.
    telo = re.sub(
        r'<aside class="document-meta">\s*<p>(.*?)</p>\s*</aside>',
        lambda m: f'<div class="note"><b>Версии</b><p>{m.group(1)}</p></div>',
        telo, flags=re.S,
    )

    # --- источники: последний раздел исходника ------------------------------
    # Здесь они не сноски, а сгруппированные списки ссылок. Забираем их
    # в мету и строим общий блок сайта — иначе плашка «Как читать» скажет
    # «по 0 источникам», а список внизу будет свёрстан не как у соседей.
    istochniki, hvost = {}, ''
    for m in re.finditer(r'<h2 id="([^"]+)"[^>]*>(.*?)</h2>', telo, re.S):
        if 'сточник' in tekst(m.group(2)):
            hvost, telo = telo[m.end():], telo[:m.start()]
    gruppa = ''
    for m in re.finditer(
        r'<h3[^>]*>(.*?)</h3>|<li><a href="([^"]+)"[^>]*>(.*?)</a></li>', hvost, re.S
    ):
        if m.group(1) is not None:
            gruppa = tekst(m.group(1))
            continue
        kod = str(len(istochniki) + 1)
        nazv = tekst(m.group(3))
        istochniki[kod] = {
            'title': f'{gruppa} — {nazv}' if gruppa and gruppa not in nazv else nazv,
            'url': m.group(2),
        }
    if not istochniki:
        print('  ВНИМАНИЕ: раздел с источниками не найден — блок внизу будет пуст')

    # --- таблицы: обернуть в прокручиваемый блок ----------------------------
    # В исходнике таблицы голые. Без обёртки таблица на 4 колонки тянет
    # горизонтальную прокрутку всей страницы на телефоне.
    telo = re.sub(r'<table>(.*?)</table>',
                  r'<div class="tbl"><table>\1</table></div>', telo, flags=re.S)

    # --- ссылки, код, мусор чужой оснастки ----------------------------------
    telo = re.sub(
        r'<a href="(https?://[^"]+)"[^>]*>',
        r'<a class="link" href="\1" target="_blank" rel="noopener">',
        telo,
    )
    telo = re.sub(r'<code class="language-[a-z]*">', '<code>', telo)
    telo = re.sub(r'<button class="back-top".*?</button>', '', telo, flags=re.S)

    # --- список источников внизу: та же вёрстка, что у остальных гайдов -----
    punkty = ''.join(
        f'<li id="src-{kod}"><span class="sn">{kod}</span><div>{html.escape(v["title"])}'
        f'<a class="link" href="{v["url"]}" target="_blank" rel="noopener">{v["url"]}</a>'
        f'</div></li>'
        for kod, v in istochniki.items()
    )
    telo += f'<h2 id="istochniki">Источники</h2><ol class="srclist">{punkty}</ol>'

    # --- оглавление: только разделы -----------------------------------------
    # 76 подразделов дали бы список на 93 строки. Решение Глеба: один уровень.
    toc = [
        {'href': '#' + m.group(1), 'label': tekst(m.group(2)), 'uroven': 1}
        for m in re.finditer(r'<h2 id="([^"]+)"[^>]*>(.*?)</h2>', telo, re.S)
    ]
    ...
    meta = {
        'slug': SLUG,
        # название в исходнике — длинная строка с двоеточием («3x-ui и Xray:
        # технический справочник по…»). В каталоге сайта заголовок короткий,
        # а пояснение уходит в подводку, поэтому правленое название бережём.
        'title': BYLO.get('title') or nazvanie,
        'lede': BYLO.get('lede', ''),
        'oblast': BYLO.get('oblast', ''),
        'data': BYLO.get('data', 'Август 2026'),
        'god': BYLO.get('god', '2026'),
    ...
    os.makedirs(os.path.join(KOREN, 'src/content/gaidy'), exist_ok=True)
    os.makedirs(os.path.join(KOREN, 'src/data/gaidy'), exist_ok=True)
    open(os.path.join(KOREN, f'src/content/gaidy/{SLUG}.html'), 'w',
         encoding='utf-8').write(telo)
    open(os.path.join(KOREN, f'src/data/gaidy/{SLUG}.json'), 'w',
         encoding='utf-8').write(json.dumps(meta, ensure_ascii=False, indent=1))

    for pole, chto in (('lede', 'подводка'), ('oblast', 'область'),
                       ('kak_chitat', 'текст плашки «Как читать»')):
        if not meta[pole]:
            print(f'  ВНИМАНИЕ: {chto} пуста — впишите руками '
                  f'в src/data/gaidy/{SLUG}.json, следующий прогон её сохранит')

    print(f'{SLUG}: разделов {len(toc)}',
          f'· подразделов {len(re.findall(r"<h3", telo))}',
          f'· таблиц {len(re.findall(r"<table", telo))}',
          f'· блоков кода {len(re.findall(r"<pre", telo))}',
          f'· источников {len(istochniki)} · слов {slov}')
